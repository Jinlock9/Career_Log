...
bool MachineSinking::PostponeSplitCriticalEdge(MachineInstr &MI,
  ...
  if (isWorthBreakingCriticalEdge(MI, FromBB, ToBB, DeferredFromBB)) {
    ...
    if ((!DeferredFromBB ||
      ...
      if (DeferredFromBB)
        ToSplit.insert(std::make_pair(DeferredFromBB, ToBB));
      LLVM_DEBUG(dbgs() << "Marking critical edge (" << FromBB->getName()
                        << " -> " << ToBB->getName() << ") for splitting.\n");
      Status = true;
    } else {
      LLVM_DEBUG(dbgs() << "Skipping critical edge (" << FromBB->getName()
                        << " -> " << ToBB->getName()
                        << ") as it is not legal or profitable to split.\n");
    }
  }
  ...
  for (const MachineOperand &MO : MI.operands()) {
    ...
    if (Reg.isPhysical()) {
      ...
      for (MachineBasicBlock *SuccBlock :
        ...
        if (SuccBlock->pred_size() > 1 && !ToSplit.count(std::pair(MBB, SuccBlock))) {
          LLVM_DEBUG(dbgs() << "Skipping" << SuccBlock->getName() << "since the critical edge was not split.\n");
          continue;
        }
...
bool MachineSinking::SinkInstruction(MachineInstr &MI, bool &SawStore,
  ...
  if (SuccToSinkTo->pred_size() > 1) {
    ...
    //   return false;
    // }
    bool EdgeWasSplit = ToSplit.count(std::make_pair(ParentBlock, SuccToSinkTo));

    if (!EdgeWasSplit) {
      LLVM_DEBUG(dbgs() << " *** NOT sinking: Edge was NOT split ***\n");
      return false;
    }
  }
